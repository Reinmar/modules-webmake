...
module.exports = function (input, options, cb) {
	...
	var templatePath = options.templatePath ? options.templatePath : __dirname + '/webmake.tpl';
	...
	(function (path) {
		...
		(function (tpl) {
			var src = tpl
				.replace(/%%modules%%/, modulesToString(parser.modules))
				.replace(/%%start_path%%/, stringify(path));
